...
x^2 & for $\int x\,dx>0$\\
\int x^3\,x & else
\end{drcases*} \quad \Rightarrow \cdots
\]
...
\]
\reDeclarePairedDelimiterInnerWrapper\abs{star}{#1#3#2}
\reDeclarePairedDelimiterInnerWrapper\abs{nostar}{#2#1#3}
\[
\abs{\pi} \quad \abs*{\phi}
\]
...
\[
\lnorm{\frac{\pi}{\int x dx}} \quad \Prop{foo \given bar}
\]
